...
export function resolveHomeDestination(action: {
  action_id?: string;
  value?: string;
  ...
  return (
    ...
    (action.action_id?.startsWith("nobo_home_open_") ? action.value : undefined)
  );
}
export function registerSlackHomeActions(
  bolt: App,
  dependencies = {
    access: evaluateNoboAccess,
    publish: publishSlackAppHome,
    details: createSlackHomeDetailsView,
  },
) {
  bolt.action(/^nobo_home_/, async ({ ack, action, body, client }) => {
    ...
    if (action.action_id === "nobo_home_refresh") {
      try {
        await dependencies.publish(body.user.id);
      } catch (error) {
        recordOpsError("home refresh", error);
      }
      return;
    }
    ...
    const opening = {
      ...
      view: modal("NoBo tools", textBlocks("Loading…")),
    };
    const opened =
      "view" in body && body.view?.type === "modal"
        ? await client.views.push(opening)
        : await client.views.open(opening);
    if (!opened.view?.id) return;
    const show = (view: ViewsOpenArguments["view"]) =>
      client.views.update({ view_id: opened.view!.id!, view });
    try {
      const selected = resolveHomeDestination(action);
      ...
      if (action.action_id === "nobo_home_cancel") {
        ...
        await show(
          modal(
            "Reminder cancelled",
            textBlocks(
              "Future occurrences stopped. A delivery already in progress may still finish.",
            ),
          ),
        );
            ...
            [
              {
                type: "input",
                block_id: "text",
                label: plain("Reminder text"),
                element: {
                  type: "plain_text_input",
                  action_id: "value",
                  multiline: true,
                  max_length: 1000,
                  initial_value: schedule.task,
                },
              },
              {
                type: "input",
                block_id: "when",
                label: plain("Next occurrence"),
                element: {
                  type: "datetimepicker",
                  action_id: "value",
              ...
              ...textBlocks(
                `Timezone: ${escapeSlack(schedule.timezone)}. Cadence and destination stay unchanged.`,
              ),
            ],
            {
              callback_id: "nobo_home_edit_submit",
              private_metadata: id,
              submit: plain("Save"),
            },
          ),
        ...
        const blocks = items.flatMap((item) => [
          ...textBlocks(
            escapeSlack(item.summary) +
              "\nNext: " +
              escapeSlack(item.nextRunAt),
          ),
          {
            type: "actions",
            elements: [
              {
                ...widgetButton("Edit", "unused", item.id),
                action_id: "nobo_home_edit",
              },
              {
                ...widgetButton(
                  "Cancel",
        ...
        await show(
          modal(
            ...
            blocks.length
              ? blocks
              : textBlocks(
                  "No active reminders. Use New reminder on NoBo Home to create one.",
                ),
          ),
        );
      } else if (selected === "artifacts") {
        const items = await listRecentArtifacts(15, {
        ...
        await show(
          modal(
            ...
            items.length
              ? items.flatMap((a) =>
                  textBlocks(
                    `<${a.previewUrl}|${escapeSlack(a.title).replace(/\|/g, " ")}>\n${a.kind} · Updated ${a.updatedAt}\nID: ${a.id.slice(0, 8)}`,
                  ),
                )
              : textBlocks(
                  "No saved documents yet. Use Save note on a response.",
                ),
          ),
        );
          ...
          modal(
            ...
            [
              ...
              },
              {
                type: "input",
                block_id: "model",
                label: plain("Model override"),
                element: {
                  type: "static_select",
                  action_id: "value",
                  options: [
                    { text: plain("Use workspace default"), value: "default" },
                    ...definitions.map((model) => ({
                      text: plain(
                        model.id +
                          (supportsOpenCodeGoImageInput(model.id)
                            ? " · images"
                            : "") +
                          (requiresOpenCodeGoDataTrainingOptIn(model.id)
                            ? " · training opt-in"
                            : ""),
                      ),
                      value: model.id,
        ...
        await show(modal("Integrations", textBlocks(integrationStatus())));
      else
        await show(
          modal(
            "NoBo Home",
            textBlocks(
              "This shortcut is no longer available. Close this window and refresh NoBo Home.",
            ),
          ),
        );
    } catch (error) {
      ...
      await show(
        modal(
          "Unable to complete",
          textBlocks(escapeSlack(summarizeOpsError(error))),
        ),
      );
    }
  });
  bolt.view(/^nobo_home_.*_submit$/, async ({ ack, body, view, client }) => {
    ...
    const when = (
      values.when?.value as { selected_date_time?: number } | undefined
    )?.selected_date_time;
    ...
    if (edit) {
      if (!text.trim() || text.length > 1000)
        errors.text = "Enter 1–1000 characters.";
      if (!when || !Number.isFinite(when) || when * 1000 <= Date.now())
        errors.when = "Choose a future time.";
    } else {
      if (!channel || !/^[CG][A-Z0-9]+$/.test(channel))
        errors.channel = "Choose a channel.";
      if (model !== "default" && !normalizeOpenCodeGoSupportedModelId(model))
        errors.model = "Choose a supported model.";
    }
    ...
    await ack({
      ...
      view: modal("Saving", textBlocks("Saving your change…")),
    });
    try {
      ...
      if (edit)
        await editOwnedSchedule(
          view.private_metadata,
          body.user.id,
          text,
          new Date(when! * 1000).toISOString(),
        );
      else {
        await assertSlackTargetChannelAllowed({
          userId: body.user.id,
        ...
        const result =
          model === "default"
            ? await clearChannelModelPreference(channel!)
            : await setChannelModelPreference(channel!, model!);
      ...
      await client.views.update({
        ...
        view: modal(
          "Saved",
          textBlocks(edit ? "Reminder updated." : "Channel model updated."),
        ),
      });
      await publishSlackAppHome(body.user.id);
    } catch (error) {
