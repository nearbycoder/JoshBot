...
- Lookup uses the public [FxEmbed API v2](https://docs.fxembed.com/api/introduction/); downloads come only from X's image/video CDNs. No X API key, cookies, account connection, AI model, or new hosted infrastructure is needed. This **does depend on an external public service**: FxEmbed receives the public post ID and server IP, never Slack tokens or conversation content. Service outages, rate limits, and X changes can make posts unavailable.
