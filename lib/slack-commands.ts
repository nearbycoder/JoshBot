...
export async function handleSlackSlashCommandPayload(
  ...
  if (command !== "/nobo-help") {
    return immediate(
      ephemeral(
        "This endpoint is configured for `/nobo-help`, `/nobo-status`, `/nobo-admin`, `/nobo-search`, `/nobo-listen`, `/nobo-prefs`, `/nobo-memory`, `/nobo-artifacts`, `/nobo-decisions`, `/nobo-issues`, `/nobo-polls`, `/nobo-news`, `/nobo-hacker-news`, `/nobo-ai-news`, `/nobo-channel-digest`, `/nobo-channel-model`, and `/nobo-dad-joke`. Try `/nobo-help`."
      )
    );
  }
